...
(function ($) {
  ...
  // Periksa apakah elemen dengan kelas 'brand-link' ada sebelumnya
  var $brandLink = $(".brand-link");
  if ($brandLink.length > 0) {
    // Dapatkan elemen pertama dengan kelas 'brand-link'
    var brandLink = $brandLink[0];

    // Pastikan elemen ditemukan sebelum mencoba mengakses kelas-kelasnya
    if (brandLink !== undefined && brandLink !== null) {
      brandLink.classList.forEach(function (className) {
        if (active_brand_color === null && logo_skins.indexOf(className) > -1) {
  ...
  if (active_brand_color) {
    // Perbarui elemen dengan kelas yang sesuai
    var $brandVariants = $(".brand-variants");
    $brandVariants.find("option." + active_brand_color).prop("selected", true);
    $brandVariants
      .removeClass()
      .addClass("custom-select mb-3 text-light border-0 ")
